...
class CensorLogsService < BaseLogsService
  ...
  def call
    ...
    Rails.logger.info("Censoring logs ids: #{@log_ids.inspect}")

    update_logs_status(@log_ids, :processing)

    @log_ids.each { |log_id| process_log_id(log_id) }

    Rails.logger.info("Censored logs ids: #{@log_ids.inspect}. elapsed=#{Time.zone.now - service_start_time}")
  end
  ...
  def process_log_id(log_id)
    ...
    content = File.read(File.join(@logs_path, filename))
  ...
  def store_scan_report(contents, remote_log, log, findings, censored_content)
    remote_log.store_scan_report(contents, findings)
