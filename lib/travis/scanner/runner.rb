  ...
  module Scanner
    class Runner
      def run(logs_path)
        ...
        plugin_results = @scanner_plugins.map { |scanner_plugin| scanner_plugin.result }

        logs = {}
        plugin_results.each do |scanner_result|
      ...
      private
      def scanner_plugins
        enabled_plugins = Settings.plugins.filter { |key, value| value.enabled != false }
        @scanner_plugins ||= enabled_plugins.map { |key, value| "Travis::Scanner::Plugins::#{key.to_s.camelize}".constantize.new(value['cmdline'], value) }
      end
    end
  end
end
